...
public class ImplCalculatorByStack {
	...
	public static void main(String[] args) {
		...
		// 定义一个字符串
//		String expression = "30+2*60-2-50+2*90";
		...
		while(true){
			...
			if(operStack.isOper(ch)){  // 如果是运算符的话
				...
				if(!operStack.isEmpty()){
					...
					// 在从符号栈中pop出一个符号，进行运算，将得到的结果入数栈，然后将当前的操作符入符号栈
				...
				// 如果是数，则直接入数栈
				//numStack.push(ch - 48);  // 这里由于得到的是字符'1',而字符'1'对应的ascii码表值与知道的值相差48
				
				// 多位数的运算：
					// 1.当处理多位数时，不能发现是一个数就立即入栈，因为它可能是多位数
					// 2.在处理数时，需要向expression的表达式的index后再看一位，如果是数就进行扫描，如果是符号入栈
					// 3.因此需要我们定义一个变量 字符串，用于拼接
		...
		// 表达式扫描完毕，就开始处理数据 -> 就顺序的从数栈和符号栈中pop各自的符号和数字，运行计算
		while(true){
		...
		int resL = numStack.pop();
		// 打印一下表达式和结果
		System.out.printf("表达式 %s = %d",expression,resL);
